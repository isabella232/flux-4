...
RUN make build/fluxd build/kubectl build/sops build/kustomize 
...
COPY --from=builder /src/docker/known_hosts.sh /home/flux/known_hosts.sh
...
COPY --from=builder /src/docker/ssh_config /etc/ssh/ssh_config

COPY --from=builder /src/build/kubectl /usr/local/bin/
COPY --from=builder /src/build/kustomize /usr/local/bin
COPY --from=builder /src/build/sops /usr/local/bin
...
COPY --from=builder /src/docker/kubeconfig /root/.kube/config
COPY --from=builder /src/build/fluxd /usr/local/bin/
